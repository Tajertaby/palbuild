...
class SSDScraper:
    ...
    @classmethod
    async def _fetch_ssd_content(
        cls, url: str, tag_names: List[str], class_list: Union[str, List[str]]
    ) -> BeautifulSoup:
        """Fetch and parse HTML content."""
        return await cls.fetcher.fetch_html_content(url, tag_names, class_list)

    @classmethod
    def _extract_ssd_basic_info(
    ...
    def _validate_and_format_results(
        names: List[str], released: List[str], capacities: List[str], urls: List[str]
    ...
    def _process_ssd_variants(cls, variant_element: BeautifulSoup) -> str:
        ...
        for li in variant_element.select("ul:not([style]) li"):
            ...
            capacities = [
                ...
                for cap in li.select("span.variants-list--item")
            ]
        ...
        for section in soup.find_all("section"):
            ...
            for prop in specs_mapping[table_name]:
                ...
                if prop != "Hardware Versions:":
                    ...
                    warning = "⚠️ **Warning:** This SSD has multiple unannounced hardware swaps which impacts performance.\n\n"
                    message_parts.insert(
                        0, f"{warning}**__Variants__**\n{variants}\n\n"
                    )
